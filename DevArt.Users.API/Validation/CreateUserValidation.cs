...
public class CreateUserValidation : AbstractValidator<CreateUserDto>
{
    ...
    {
        RuleFor(user => user.Email).EmailAddress().WithMessage("Your email address should be a valid email.");
        ...
        RuleFor(user => user.Password).Equal(user => user.PasswordConfirmation)
